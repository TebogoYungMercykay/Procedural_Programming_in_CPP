...
int main() {
  ...
  fstream streamWriterObject(tapeTextFile.c_str()),
      streamReaderObject(fileTextFile.c_str());
  while (k_iterator > 10) {
    streamReaderObject >> tempvar;
    sizevar = tempvar.length();
    arrayVAr = new string[sizevar];
    counterVar = 0;
    for (; counterVar < sizevar;) {
      arrayVAr[counterVar] = tempvar[counterVar];
      counterVar += 1;
    }
...
void resizeArray() {
  ...
  for (counterVar = 0; counterVar < sizevar;) {
    if (counterVar == (sizevar - 1))
      arrayVAr[counterVar] = "U";
    else
      arrayVAr[counterVar] = tempvarArray[counterVar];
    counterVar += 1;
  }
}
void TuringMachine(string tempvarString) {
  ...
  if (tempvarString == "fifthState") {
    if (arrayVAr[indexvar] == "P" || arrayVAr[indexvar] == "X") {
      indexvar -= 1;
      TuringMachine("fifthState");
    } else if (arrayVAr[indexvar] == "U") {
      if (indexvar == sizevar - 1)
