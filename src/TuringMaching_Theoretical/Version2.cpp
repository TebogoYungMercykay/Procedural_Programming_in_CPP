...
void change_state(std::string changeState) {
  if (changeState == fir) {
    ...
    if (statearray[indexvar] == "P" || statearray[indexvar] == "X") {
      indexvar = indexvar - 1;
      change_state(five);
    } else if (statearray[indexvar] == "U") {
      if (indexvar == sizevar - 1)
...
int main() {
  std::fstream streamWriterObject(tapeTextFile.c_str()),
  ...
  if (streamReaderObject.is_open()) {
    ...
    countState = 0;
    while (countState < sizevar) {
      statearray[countState] = statetempvar[countState];
      countState = countState + 1;
    }
    change_state(fir);
    ...
    countState = 0;
    while (countState < sizevar) {
      std::cout << statearray[countState];
      countState = countState + 1;
    }
    std::cout << std::endl;
